...
├── README.md               # This file
├── data/                   # NOT uploaded to GitHub — download separately (see below)
│   └── spam.csv
└── outputs/                # NOT uploaded to GitHub — generated by running analysis.py
...
**Note:** Only `analysis.py`, `requirements.txt`, and `README.md` need to be uploaded to GitHub. The `data/` and `outputs/` folders are excluded — the dataset must be downloaded separately and the outputs are regenerated by running the script.
...
Download the SMS Spam Collection from [Kaggle](https://www.kaggle.com/datasets/uciml/sms-spam-collection-dataset) or [UCI](https://archive.ics.uci.edu/dataset/228/sms+spam+collection). Unzip the download and place `spam.csv` inside a `data/` folder:
...
Almeida, T. A., Gómez Hidalgo, J. M., and Yamakami, A. (2011). Contributions to the study of SMS spam filtering: new collection and results. *Proceedings of the 11th ACM Symposium on Document Engineering*, Mountain View, CA, pp. 259–262.
